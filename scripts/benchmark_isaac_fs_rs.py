"""Benchmark stereo models + RealSense hardware depth against ISAC ground truth.

Uses the ISAC ``reflective_test`` dataset loaded via ``data_manager_isaac.DataSource``.
...
Usage:
  cd c:\\Work\\Code\\Fast-FoundationStereo
  python scripts/benchmark_isaac_fs_rs.py [--out_dir reports/benchmark_isaac_fs_rs]
"""
...
import Utils as U
from scripts.data_manager_isaac import DataSource
from metrics import (
    BenchmarkResults,
    FrameMetrics,
    compute_metrics,
...
def main():
    ...
    parser.add_argument('--views',    nargs='+', default=list(DEFAULT_VIEWS),
                        help='Camera views to evaluate (front/overhead/side/wrist)')
    parser.add_argument('--episodes', nargs='*', default=list(DEFAULT_EPISODES),
                        help='Episode folder names to filter on (default: all episodes)')
    ...
    logging.info(f"Found {n} samples in {args.isaac_dir}")
    if n == 0:
        logging.error("No samples found — check --isaac_dir path")
        return
    ...
    for idx in range(n):
        ...
        for mname in active_methods:
            ...
            if mname == GT_NAME:
                ...
                # RealSense hardware depth — compare against ISAC GT (both in mm)
    ...
    # ISAC GT has no latency; RealSense is hardware 30 FPS
    ...
    method_configs[RS_NAME] = {"source": "RealSense hardware depth (depth_tyzx, 30 FPS)"}
    method_configs[GT_NAME] = {"source": "ISAC reflective_test ground-truth depth (depth_left)"}
    ...
    results = BenchmarkResults(
        ...
        source=(
            f"ISAC reflective_test  •  {args.isaac_dir}  "
            f"•  views={args.views}  •  episodes={args.episodes or 'all'}"
        ),
